...
func TestSnapshotDRWAGateMetrics_Smoke(t *testing.T) {
	...
	// After reset, all counters are zero. Keys may remain in the map.
	for k, v := range snap {
		if v != 0 {
			t.Fatalf("expected counter %q to be zero after reset, got %d", k, v)
		}
	}
}
...
func TestDRWADenialMetric_AllCodesHaveMetric(t *testing.T) {
	for _, code := range coredrwa.AllDenialCodes() {
		metric := drwaDenialMetric(errors.New(string(code)))
		if metric == "" {
			t.Fatalf("drwaDenialMetric returned empty string for denial code %q — add a case to drwaDenialMetric switch", code)
		}
	}
}
